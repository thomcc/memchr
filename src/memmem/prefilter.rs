...
use crate::memmem::byte_frequencies::BYTE_FREQUENCIES;
...
impl Freqy {
    ...
    pub fn reverse(needle: &[u8]) -> Freqy {
        ...
        for (i, &b) in needle.iter().rev().enumerate().skip(2) {
            if Freqy::rank(b) < Freqy::rank(rare1) {
                rare2 = rare1;
                rare2i = rare1i;
                rare1 = b;
    ...
    pub fn find_candidate(
        ...
        while prestate.is_effective() {
            ...
            // needle.
            i += match crate::memchr(self.rare1, &haystack[i..]) {
                None => return None,
                Some(found) => {
    ...
    pub fn rfind_candidate(
        ...
        while prestate.is_effective() {
            ...
            // occurrence of the rarest byte (heuristically chosen) in the
            // needle.
            i = match crate::memrchr(self.rare1, &haystack[..i]) {
                None => return None,
                Some(found) => {
